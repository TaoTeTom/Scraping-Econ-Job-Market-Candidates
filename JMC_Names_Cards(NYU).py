...
url = url_df['Url'][5]
print(url)
school = url_df['School'][5]
# requests data from url
html = requests.get(url).content
soup = BeautifulSoup(html, features="lxml")

candidate_card_content_classes = soup.find_all('div', class_='columncontrol section')
candidate_card_img_classes = soup.find_all('div', class_='image parbase section')
...
def get_card_content(candidate_card_content_class):
    name_element = candidate_card_content_class.find("b").text
    #print("Name:", name_element)
    website_element = candidate_card_content_class.find_all('a',
                                                          attrs={'href': re.compile("^http")})[1].get('href')
    ...
    fos_element = candidate_card_content_class.find_all("p")[1].text
